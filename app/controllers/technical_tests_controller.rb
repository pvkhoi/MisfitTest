class TechnicalTestsController < ApplicationController
  before_filter :authenticate_user!, only: [:index, :edit, :destroy, :report]
  before_filter :set_technical_test, only: [:show, :edit, :update, :destroy, :start, :update_start_time, :finish, :update_finish]
  before_filter :check_conditions_for_show, only: [:show]
  before_filter :check_test_available, only: [:show, :start, :update_start_time]
  before_filter :check_update_start_time, only: [:start, :update_start_time]
